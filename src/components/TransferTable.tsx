...
import type { TimelineItem, TransferEvent } from '../chain/transfers'
...
export function TransferTable({
  ...
  if (items.length === 0) {
    return <p className="muted">No recent transfers found.</p>
  }
        ...
        <tbody>
          {items.map((item) => {
            if (item.kind === 'transfer') {
              return (
                <TransferRow
                  key={`${item.txHash}-${item.from}-${item.to}-${item.token.address}-${item.blockNumber}`}
                  item={item}
                  self={self}
                  networkId={networkId}
                />
            ...
            return (
              <tr key={`${item.txHash}-${item.eventName}-${item.paymentId}`}>
                <td>
